...
<div class="activity">
    <div class="activity-item" :class="activeType[support.type]" v-for="(support,index) in info.supports" :key='index'>
...
export default {
    ...
    mounted(){
      // ul的宽度没有指定样式，默认从父类继承，所以ul的宽度没有被撑开，不能形成横向滑屏
      // 如果info.pics没有数据，则先不设置滑屏
      if(!this.info.pics){
        return false
      }
      
      // 如果info是请求成功后的数据,则设置ul的宽度并设置滑屏
      this.setUlWidth()
      this._intiScroll()
    },
    computed:{
    ...
    methods:{
      ...
      setUlWidth(){
        ...
        ul.style.width = (liWidth+marginLeft)*size + 'px'
      }
    },

    watch:{
      info(){
        // 如果是先获取到空的info，然后再返回带数据的info，则再info变化后再设置滑屏
        this.$nextTick(() =>{
          // 数据更新后立即设置ul的宽度
            this.setUlWidth()
            this._intiScroll()
