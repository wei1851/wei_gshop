        ...
        <div class="ratings-content">
            ...
            <div class="ratingselect">
                <div class="rating-type border-1px">
                    ...
                    满意<span class="count">{{positiveRating}}</span>
                    </span>
                    <span class="block negative" @click="setSelectType(1)" :class="{active:selectType===1}">
                    不满意<span class="count">{{info.ratingCount-positiveRating}}</span>
                    </span>
                </div>
            ...
            <div class="rating-wrapper">
                <ul>
                    <li class="rating-item" v-for="(rating,index) in fliterRatings" :key="index">
                        <div class="avatar">
                            <img width="28" height="28" :src="rating?rating.avatar:''">
                        </div>
                        <div class="content">
                            ...
                            <div class="recommend">
                                ...
                                <span class="item" v-for="(tag,index) in rating.recommend" :key="index">{{tag}}</span>
                            </div>
...
export default {
    data(){
      return{
        selectType:2,//2是全部，0是满意，1是不满意
        onlyContent:false//规定查看内容是否存在
      }
    },
    ...
    computed:{
      ...
      fliterRatings(){
        const {ratings} = this
        // 如果没有数据，则返回空数组，防止报错，常用在ajax里返回的属性
        if(!ratings){
          return []
        }
        ...
        if(selectType === 2){
          // 如果onlyContent为false，所有评论都看，返回所有结果，!onlyContent直接返回
          return !onlyContent || rating.text.length>0
        }else{
          return rating.rateType === selectType && (!onlyContent || rating.text.length>0)
        }
